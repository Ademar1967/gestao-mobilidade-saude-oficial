            ...
            if os.path.exists(caminho):
                ...
                try:
                    with open(caminho, encoding='utf-8') as f:
                        reader = csv.reader(f)
                        linhas = list(reader)
                        self.stdout.write(f'  Cabeçalho: {linhas[0] if linhas else "(vazio)"}')
                        for i, linha in enumerate(linhas[1:4], 1):
                            self.stdout.write(f'  Linha {i}: {linha}')
                except Exception as e:
                    self.stdout.write(self.style.ERROR(f'  [ERRO] Falha ao ler CSV: {e}'))
            else:
                self.stdout.write(self.style.WARNING('  [FALTA] Arquivo não encontrado.'))
