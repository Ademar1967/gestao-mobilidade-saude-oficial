...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "arquivo", type=str, help="Caminho do arquivo CSV ou Excel com os pacientes"
        )
    ...
    def handle(self, *args, **options):
        ...
        for _, row in df.iterrows():
            ...
            try:
                ...
                self.stderr.write(
                    self.style.WARNING(
                        f"Erro ao importar linha: {row.to_dict()} | Erro: {e}"
                    )
                )
        self.stdout.write(
            self.style.SUCCESS(f"Importação concluída. {criados} pacientes importados.")
        )
