...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not os.path.exists(csv_path):
            self.stdout.write(
                self.style.WARNING("condutores.csv não encontrado, pulando.")
            )
            return
            ...
            for row in reader:
                ...
                if nome:
                    ...
                    if created:
                        count += 1
                        self.stdout.write(
                            self.style.SUCCESS(f"Condutor criado: {nome}")
                        )
                    else:
                        self.stdout.write(f"Condutor já existe: {nome}")
        self.stdout.write(
            self.style.SUCCESS(
                f"Importação concluída. Total de novos condutores: {count}"
            )
        )
