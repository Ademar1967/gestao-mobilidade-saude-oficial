...
def sync_master_data_csvs() -> None:
    ...
    clinicas_rows = [
        ...
        for c in Clinica.objects.order_by('nome').only('nome', 'endereco', 'bairro', 'cidade', 'telefone')
    ]
    _write_csv(base_dir / 'clinicas.csv', ['Nome', 'Endereco', 'Bairro', 'Cidade', 'Telefone'], clinicas_rows)
    ...
    viaturas_rows = [
        ...
        for v in Veiculo.objects.order_by('id').only('patrimonio', 'placa', 'tipo_veiculo', 'lotacao')
    ]
    _write_csv(base_dir / 'viaturas.csv', ['patrimonio', 'placa', 'tipo_veiculo', 'lotacao'], viaturas_rows)
