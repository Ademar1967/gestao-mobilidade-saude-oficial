...
class AutocompleteProtecaoTest(TestCase):
    def test_exportacao_pacientes_exige_login(self):
        ...
        self.assertIn(
            resp.status_code,
            [302, 401],
            "Exportação de pacientes deveria exigir autenticação!",
        )
    ...
    def test_cookie_sessao_seguro(self):
        ...
        resp = self.client.post(
            login_url,
            {"username": "teste_autocomplete", "password": "segredo123"},
            follow=True,
        )
        ...
        self.assertIsNotNone(
            session_cookie, "Cookie de sessão não encontrado após login!"
        )
        # Verifica flags de segurança
        self.assertTrue(
            session_cookie.get("httponly", False), "Cookie de sessão deve ser HttpOnly!"
        )
        self.assertTrue(
            session_cookie.get("secure", False), "Cookie de sessão deve ser Secure!"
        )
        self.assertIn(
            session_cookie.get("samesite", "").lower(),
            ["lax", "strict"],
            "Cookie de sessão deve ter SameSite Lax ou Strict!",
        )
    ...
    def test_endpoints_sensiveis_exigem_login(self):
        ...
        for url in endpoints:
            ...
            self.assertIn(
                resp.status_code,
                [302, 401],
                f"Endpoint {url} deveria exigir autenticação!",
            )
