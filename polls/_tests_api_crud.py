...
class APICrudTests(APITestCase):
    ...
    def test_paciente_crud(self):
        ...
        # Update
        response = self.client.patch(f'/api/pacientes/{paciente_id}/', {'idade': 41}, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        # Delete
    ...
    def test_clinica_crud(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        response = self.client.patch(f'/api/clinicas/{clinica_id}/', {'nome': 'Clínica Atualizada'}, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_condutor_crud(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        response = self.client.patch(f'/api/condutores/{condutor_id}/', {'nome': 'Condutor Atualizado'}, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_enfermagem_crud(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        response = self.client.patch(f'/api/enfermagens/{enfermagem_id}/', {'nome': 'Enfermagem Atualizada'}, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        response = self.client.delete(f'/api/enfermagens/{enfermagem_id}/')
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)

    def test_veiculo_crud(self):
        data = {'tipo_veiculo': 'ambulancia', 'placa': 'ABC1234', 'patrimonio': 'P001', 'lotacao': 2}
        response = self.client.post('/api/veiculos/', data, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        veiculo_id = response.data['id']
        response = self.client.get('/api/veiculos/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        response = self.client.patch(f'/api/veiculos/{veiculo_id}/', {'lotacao': 3}, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
