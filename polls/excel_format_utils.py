...
def formatar_excel_profissional(nome_arquivo):
            ...
            for row in ws.iter_rows(min_row=2, min_col=idx, max_col=idx):
                for cell in row:
                    ...
                    cell.comment = Comment(
                        f"Orientação: {instrucoes[col_name]}", "Sistema"
                    )
    ...
    for idx, cell in enumerate(ws[1], 1):
        if str(cell.value).strip() == "Evolução":
            dv = DataValidation(
                type="list",
                formula1='"Estável,Crônico,Em tratamento"',
                allow_blank=True,
            )
    ...
    titulo.fill = PatternFill(
        start_color="B3D1F2", end_color="B3D1F2", fill_type="solid"
    )
    ...
    try:
        ...
        print(
            f"ERRO: Não foi possível salvar o arquivo {nome_arquivo}. Feche o arquivo no Excel e execute novamente."
        )
